...
# Título da aplicação
st.title("Gerador de Desenhos para Pintar")
...
if st.button("Gerar Desenho"):
    ...
    if APIK and tema:
        ...
        prompt = f"Crie uma imagem de desenho em preto e branco, com linhas finas e contornos bem definidos, adequada para ser colorida. O desenho deve representar {tema}. Certifique-se de que as áreas a serem pintadas sejam claras e distintas, facilitando o processo de colorir. O fundo deve ser branco."
        ...
        try:
            ...
            # Campo para inserir o nome do arquivo PDF
            savename = st.text_input("Nome do arquivo PDF (ex: desenho.pdf):")
            
            # Botão para salvar a imagem em PDF
            if st.button("Salvar em PDF"):
                if savename:
                    ...
                    st.success(f"Desenho salvo como {savename}")
                else:
                    st.warning("Por favor, forneça um nome para o arquivo PDF.")
